...
noaa_future_daily <- noaa_future |> 
  ...
  # convert to Celsius
  mutate(air_temperature = air_temperature - 273.15) |> 
  mutate(air_pressure = air_pressure)
...
fit <- lm(site_target$temperature ~ site_target$air_temperature + log(site_target$air_pressure))
    
# use linear regression to forecast water temperature for each ensemble member
forecasted_temperature <- fit$coefficients[1] + fit$coefficients[2] * noaa_future_site$air_temperature +
    fit$coefficients[3] * log(noaa_future_site$air_pressure)
...
ggplot(targets_lm, aes(x=air_pressure, y= temperature, colour = site_id)) +
  geom_point(alpha = 0.5) +
  geom_smooth(method = 'lm') +
  theme_bw() +
  facet_wrap(~site_id)
